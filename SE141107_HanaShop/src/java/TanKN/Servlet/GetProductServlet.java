...
public class GetProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<ProductsDTO> list= (List<ProductsDTO>) (session.getAttribute("SEARCHRESULT"));
            if(list==null){
                list=new ArrayList<>();
            }
            ProductsDAO dao= new ProductsDAO();
            dao.searchProduct("", "",0 ,999999999,0 );
            list=dao.getProductList();
            session.setAttribute("SEARCHRESULT", list);
            session.setAttribute("pageNum", "1");
        
        }catch(SQLException ex){
            log("GetProductServlet_SQLException "+ex.getMessage());
        }catch(NamingException ex){
            log("NamingException_GetProductServlet "+ex.getMessage());
        }finally{
